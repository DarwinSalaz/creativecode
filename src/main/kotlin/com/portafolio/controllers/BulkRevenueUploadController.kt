...
import com.portafolio.services.BulkRevenueUploadService
import com.portafolio.services.BulkRevenueUploadValidationService
...
class BulkRevenueUploadController {
    ...
    @Autowired
    lateinit var bulkRevenueUploadService: BulkRevenueUploadService
    ...
    @GetMapping("/bulk-revenue-upload/template")
    fun downloadTemplate(): ResponseEntity<Any> {
        return try {
            val headers = listOf(
                "revenue_type", "value", "revenue_date", "justification"
            )

            val exampleRow = listOf(
                "Efectivo Carteras", "150000", "01/15/2024", "Cobro a clientes"
            )

            ResponseEntity.ok(mapOf(
                "message" to "Formato de archivo Excel requerido para ingresos",
                "headers" to headers,
                ...
                "instructions" to listOf(
                    ...
                    "Tipos de ingreso válidos: Efectivo Carteras, Venta Insumos Y Materiales, Abono Deuda, Venta Directa, Liquidación Parcial Cartera, Otros"
                )
            ))
        } catch (e: Exception) {
    ...
    @PostMapping("/bulk-revenue-upload/validate")
    fun validateFile(
        ...
        try {
            ...
            val request = BulkRevenueUploadRequest(walletId = walletId, records = records)
            val validation = validationService.validateBulkRevenueUpload(request)
    ...
    @PostMapping("/bulk-revenue-upload/process")
    fun processBulkUpload(
        @RequestBody request: BulkRevenueUploadRequest,
        @RequestHeader("Authorization") authorization: String
    ): ResponseEntity<Any> {
        try {
            ...
            val result = bulkRevenueUploadService.processBulkRevenueUpload(request, applicationUsername)
    ...
    private fun parseExcelFile(inputStream: InputStream): List<RevenueUploadRecord> {
        val workbook = try {
        ...
        val records = mutableListOf<RevenueUploadRecord>()
        ...
        for (i in 1..sheet.lastRowNum) {
            ...
            try {
                ...
                val record = RevenueUploadRecord(
                    revenueType = getCellValueAsString(row.getCell(0)),
                    value = getCellValueAsDouble(row.getCell(1)),
                    revenueDate = getCellValueAsString(row.getCell(2)),
